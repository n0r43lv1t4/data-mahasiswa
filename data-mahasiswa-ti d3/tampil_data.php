...
<div class="row flex-lg-row-reverse align-items-center mb-5">
    ...
    <div class="col-lg-4 col-xl-3">
        <a href="?halaman=entri" class="btn btn-primary rounded-pill float-lg-end py-2 px-4 mb-4 mb-lg-0">
            <i class="fa-solid fa-plus me-2"></i> Entri mahasiswa
        </a>
    </div>
    <!-- form pencarian -->
    <div class="col-lg-8 col-xl-9">
        <form action="?halaman=pencarian" method="post" class="form-search needs-validation" novalidate>
            <input type="text" name="kata_kunci" class="form-control rounded-pill" placeholder="Cari mahasiswa ..." autocomplete="off" required>
            <div class="invalid-feedback">Masukan ID atau Nama mahasiswa yang ingin Anda cari.</div>
        </form>
    </div>
</div>
...
if (isset($_GET['pesan'])) {
    ...
    if ($_GET['pesan'] == 1) {
        ...
        echo '<div class="alert alert-success alert-dismissible rounded-4 fade show mb-4" role="alert">
                <strong><i class="fa-solid fa-circle-check me-2"></i>Sukses!</strong> Data mahasiswa berhasil disimpan.
    ...
    elseif ($_GET['pesan'] == 2) {
        ...
        echo '<div class="alert alert-success alert-dismissible rounded-4 fade show mb-4" role="alert">
                <strong><i class="fa-solid fa-circle-check me-2"></i>Sukses!</strong> Data mahasiswa berhasil diubah.
                <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
            </div>';
    }
    // jika pesan = 3
    elseif ($_GET['pesan'] == 3) {
        // tampilkan pesan sukses hapus data
        echo '<div class="alert alert-success alert-dismissible rounded-4 fade show mb-4" role="alert">
                <strong><i class="fa-solid fa-circle-check me-2"></i>Sukses!</strong> Data mahasiswa berhasil dihapus.
                <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
            </div>';
    }
}
...
<div class="row mb-5">
    ...
    */
    ...
    // sql statement untuk menampilkan data dari tabel "tbl_mahasiswa"
    $query = $mysqli->query("SELECT id_mahasiswa, angkatan, nama_lengkap, foto_profil FROM tbl_mahasiswa
                             ORDER BY id_mahasiswa DESC LIMIT $batas_awal, $batas")
    ...
    if ($rows <> 0) {
        ...
        while ($data = $query->fetch_assoc()) { ?>
            ...
            <div class="p-2">
                <div class="d-flex bg-white rounded-4 shadow-sm">
                    ...
                    <div class="p-4 flex-grow-1">
                        <h5><?php echo $data['id_mahasiswa']; ?> - <?php echo $data['nama_lengkap']; ?></h5>
                        <p class="text-muted"><?php echo $data['angkatan']; ?></p>
                    </div>
                    <div class="p-4">
                        <div class="d-flex flex-column flex-lg-row">
                            <!-- button form detail data -->
                            <a href="?halaman=detail&id=<?php echo $data['id_mahasiswa']; ?>" class="btn btn-primary btn-sm rounded-pill px-3 me-2 mb-2 mb-lg-0"> Detail </a>
                            <!-- button form ubah data -->
                            <a href="?halaman=ubah&id=<?php echo $data['id_mahasiswa']; ?>" class="btn btn-success btn-sm rounded-pill px-3 me-2 mb-2 mb-lg-0"> Ubah </a>
                            <!-- button modal hapus data -->
                            <button type="button" class="btn btn-danger btn-sm rounded-pill px-3" data-bs-toggle="modal" data-bs-target="#modalHapus<?php echo $data['id_mahasiswa']; ?>"> Hapus </button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <!-- Modal hapus data -->
            <div class="modal fade" id="modalHapus<?php echo $data['id_mahasiswa']; ?>" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="modalHapusLabel" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h1 class="modal-title fs-5" id="exampleModalLabel">
                                <i class="fa-regular fa-trash-can me-2"></i> Hapus Data mahasiswa
                            </h1>
                        </div>
                        <div class="modal-body">
                            <p class="mb-2">Anda yakin ingin menghapus data mahasiswa?</p>
                            <!-- informasi data yang akan dihapus -->
                            <p class="fw-bold mb-2"><?php echo $data['id_mahasiswa']; ?> <span class="fw-normal">-</span> <?php echo $data['nama_lengkap']; ?></p>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary rounded-pill px-3" data-bs-dismiss="modal">Batal</button>
                            <!-- button proses hapus data -->
                            <a href="proses_hapus.php?id=<?php echo $data['id_mahasiswa']; ?>" class="btn btn-danger rounded-pill px-3">Ya, Hapus</a>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="d-flex flex-column flex-xl-row align-items-center mt-4">
            ...
            <div class="flex-grow-1 text-center text-xl-start text-muted mb-3">
                <?php
                // sql statement untuk menampilkan jumlah data pada tabel "tbl_mahasiswa"
                $query = $mysqli->query("SELECT id_mahasiswa FROM tbl_mahasiswa")
                ...
                */
                ...
                // sql statement untuk menampilkan jumlah data pada tabel "tbl_mahasiswa" yang ditampilkan per halaman
                $query = $mysqli->query("SELECT id_mahasiswa FROM tbl_mahasiswa LIMIT $data_awal, $batas")
